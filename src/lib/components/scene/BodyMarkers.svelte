<script lang="ts">
  ...
  /** One draw call for every body's constant-size marker, positioned relative to the camera. */

  const TINT: Partial<Record<ObjectType, string>> = {
    star: '#fff1d6',
    planet: '#ededed',
    'dwarf-planet': '#cfc6bb',
    'trans-neptunian': '#bdb3a8',
    moon: '#b4b4b4',
    asteroid: '#9d968d',
    comet: '#b7d0da',
    'earth-satellite': '#bccbd6',
    spacecraft: '#e3d2a8',
    lander: '#e3d2a8'
  };
  ...
  const material = new ShaderMaterial({
    uniforms: { uPixelRatio: { value: 1 }, uAccent: { value: ACCENT } },
    vertexShader: /* glsl */ `
      ...
      void main() {
        ...
        vSize = aSize;
        vRing = aRing;
        vExtent = aRing > 0.0 ? aRing * 2.0 + 6.0 : aSize + 4.0;
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        float halo = (1.0 - smoothstep(edge, edge + 2.0, r)) * vAlpha * 0.45;
        float ring = vRing > 0.0 ? (1.0 - smoothstep(0.35, 1.1, abs(r - vRing))) * 0.9 : 0.0;
        vec3 color = vColor * disc + uAccent * ring;
        float alpha = max(max(disc, halo), ring);
  ...
  useTask(
    ...
    () => {
      ...
      for (let i = 0; i < BODY_COUNT; i++) {
        ...
        alphas[i] = screen.markerAlpha[i];
        sizes[i] = markerSize[i];
        rings[i] =
          i === selectedIndex && screen.depth[i] > 0 && screen.radius[i] < 48
            ? Math.max(screen.radius[i], markerSize[i] / 2) + 6
            : 0;
      }
